...
const InvestorLogin = () => {
  const { loading, setLoading, status, setStatus, BASE, setInvestor } =
  ...
  const userLogin = async (e: FormEvent<HTMLFormElement>) => {
    ...
    try {
      ...
      const response = await Axios.post(`${BASE}/investors/login`, creds);
  ...
  //     try {
  //       await Axios.post(`${BASE}/pitchers/send-email`, { email });
        ...
        <div className="flex flex-col justify-center items-center h-full border border-border p-10 rounded-xl">
          ...
          <div className="w-full">
            ...
            <div className="mt-4 text-start">
              ...
              <Link to="/investoreg" className="text-blue-500 hover:underline">
                Not registered?
              </Link>
              <Link to={"/forgotpass/investor"}>Forgot Password?</Link>
            </div>
          </div>
        </div>
      </div>
